...
def make_tool_use_block(
    name="search_course_content", tool_id="tool_1", input_data=None
):
    block = MagicMock()
    block.type = "tool_use"
    block.name = name
...
def test_one_tool_round_returns_final_text(generator, mock_client, mock_tool_manager):
    ...
    result = generator.generate_response(
        "Search for Python", tools=[{}], tool_manager=mock_tool_manager
    )
...
def test_second_api_call_includes_tools(generator, mock_client, mock_tool_manager):
    ...
    generator.generate_response(
        "Query", tools=tools_list, tool_manager=mock_tool_manager
    )
...
def test_tool_result_passed_to_next_api_call(generator, mock_client, mock_tool_manager):
    tool_block = make_tool_use_block(
        "search_course_content", "tool_abc", {"query": "test"}
    )
...
def test_intermediate_api_calls_include_tools(
    generator, mock_client, mock_tool_manager
):
...
def test_tool_execution_error_does_not_propagate(
    generator, mock_client, mock_tool_manager
):
...
def test_tool_error_string_passed_as_tool_result(
    generator, mock_client, mock_tool_manager
):
    mock_tool_manager.execute_tool.side_effect = Exception("Service unavailable")
    tool_block = make_tool_use_block(
        "search_course_content", "err_tool", {"query": "test"}
    )
    mock_client.messages.create.side_effect = [
        make_response("tool_use", [tool_block]),
